 ...
 *
 *  This example shows how to use dynamic server data model with a cid file.
 *
 *  - How to open and parse the model cid file
 *  - How to access data attributes by object reference strings
 *  - How to access data attributes by short addresses
 *
 *  Note: If building with cmake the vmd-filestore folder containing the cid file
 *  (simpleIO_direct_control_goose.cid) has to be copied to the folder where the example is executed!
 *
 */
...
main(int argc, char** argv)
{
    ...
    /* parse the cid file and create the data model */
    IedModel* model = CidFileParser_createModelFromCidFileEx("simpleIO_direct_control_goose.cid");
